...
1. **Pattern engine** ([`vendetta.rs`](apps/desktop/src-tauri/src/vendetta.rs)) — 38 patterns across seven packs, each checksum-validated where one exists so a tracking number never masquerades as a card:
...
| Secrets | API keys† (OpenAI/Anthropic/Google/AWS access + secret/GitHub/GitLab/Stripe/Slack…), bearer-token headers†, JWTs, private-key blocks†, credentialed DB connection strings† | distinctive prefixes, context anchors, URI credential shape |
