...
1. **Pattern engine** ([`vendetta.rs`](apps/desktop/src-tauri/src/vendetta.rs)) — 55 patterns across seven packs, each checksum-validated where one exists so a tracking number never masquerades as a card:
...
| National IDs | US ITIN, Canadian SIN, UK NHS + National Insurance, Australian TFN, Aadhaar, Italian Codice Fiscale, Spanish DNI/NIE, Brazilian CPF, Polish PESEL, Swedish personnummer, Dutch BSN, French NIR, German Steuer-ID, Chinese resident ID | Luhn, mod-11/-97 families, Verhoeff, ISO 7064, per-scheme check digits |
